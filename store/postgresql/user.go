...
func (s *userStore) New(authService string, authID string, password string) (int64, error) {
	...
	if authService == "email" {
		err := s.db.QueryRow(
			...
			time.Now(), authService, authID, authID, password,
		).Scan(&id)
...
func (s *userStore) GetByEmail(email string) (*store.User, error) {
	row := s.db.QueryRow(selectFromUsers+` where email=$1`, email)
	return s.scanUser(row)
}

func (s *userStore) GetByEmailPassword(email string, password string) (*store.User, error) {
	row := s.db.QueryRow(selectFromUsers+` where email=$1 and password=$2`, email, password)
	return s.scanUser(row)
}
